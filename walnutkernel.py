...
class MyMagic(Magic):
    def line_showme(self, lapin):
        """
        %%showme LAPIN - display Result named lapin
        """
        try:
            import pydot
        except:
            raise Exception("You need to install pydot")
        graph = pydot.graph_from_dot_file("Result/" + str(lapin) + ".gv")[0]
...
class WalnutKernel(ProcessMetaKernel):
    ...
    def makeWrapper(self):
        child = spawn("java -jar /data/walnut.jar", echo=True, encoding="utf-8")
        child.expect(r"\n")
